...
function scrollMove(m){
	var p = document.getElementById("pic_r").getElementsByTagName("li");
	var srl = document.getElementById("pic_r").scrollLeft;
	var dsrl = Math.floor((p[0].clientWidth*(m-1)-srl)/5);
	var xsrl = Math.ceil((p[0].clientWidth*(m-1)-srl)/5);
	if(srl > p[0].clientWidth*(m-1)){
		document.getElementById("pic_r").scrollLeft = srl + dsrl;
	}else if(srl < p[0].clientWidth*(m-1)){
		document.getElementById("pic_r").scrollLeft = srl + xsrl;
	}else{
		clearInterval(scrollmove);
	}
}
